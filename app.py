...
def show_trainer_detail(trainer_name: str, rule: int, key_prefix: str = "") -> None:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown('<div class="sec">順位推移</div>', unsafe_allow_html=True)
        ...
        st.altair_chart(
            _styled(line + points, height=250),
    ...
    with c2:
        ...
        if not r_vals.empty:
            ...
            st.altair_chart(_styled(rating_chart, height=250), use_container_width=True)
        else:
